...
static FnCallResult FnCallUserInGroup(ARG_UNUSED EvalContext *ctx, ARG_UNUSED const Policy *policy, const FnCall *fp, const Rlist *finalargs)
{
    ...
    if (grent == NULL)
    {
        ...
        while (grent->gr_mem[0] != NULL)
        {
            ...
            StringSetAdd(users, group_member);
            grent->gr_mem++;
        }
    }
    
    bool result = StringSetContains(users, user_name);
    StringSetDestroy(users);

    // check primary group
    if (!result && grent != NULL)
    {
        struct passwd pwd;
        struct passwd *pwent;
        ...
        if (pwent == NULL)
        {
            switch(ret)
            {
                ...
                {
                    const char *error_msg = GetErrorStrFromCode(ret);
                    Log(LOG_LEVEL_ERR, "Couldn't open passwd database in function '%s': %s", fp->name, error_msg);
                    return FnFailure();
                }
            }
        }
