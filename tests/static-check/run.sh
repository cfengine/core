...
function create_image() {
  ...
  buildah run $c -- dnf -q -y install pcre-devel openssl1.1-devel libxml2-devel pam-devel lmdb-devel libacl-devel libyaml-devel curl-devel libvirt-devel >/dev/null 2>&1
  buildah run $c -- dnf clean all >/dev/null 2>&1
  buildah commit $c cfengine-static-checker-f$STATIC_CHECKS_FEDORA_VERSION-openssl1.1 >/dev/null 2>&1
  echo $c
}
...
if buildah inspect cfengine-static-checker-f$STATIC_CHECKS_FEDORA_VERSION-openssl1.1 >/dev/null 2>&1; then
  c=$(buildah from cfengine-static-checker-f$STATIC_CHECKS_FEDORA_VERSION-openssl1.1)
else
  c=$(create_image)
fi
