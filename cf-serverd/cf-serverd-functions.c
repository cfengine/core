...
GenericAgentConfig *CheckOpts(int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, "dvIKf:D:N:VSxLFMhA", OPTIONS, &optindex)) != EOF)
    {
        ...
        case 'A':
            ...
            if (fout == NULL)
            {
                perror("Unable to open /etc/avahi/service/cfengine-hub.service");
                exit(1);
            }
            ...
            XmlComment(writer, "This file has been automaticaly generated by cf-serverd.");
            XmlStartTag(writer, "service-group", 0);
#ifdef HAVE_NOVA
            fprintf(fout,"<name replace-wildcards=\"yes\" >CFEngine Enterprise Policy Hub on %s </name>", Version(), "%h");
#else
            fprintf(fout,"<name replace-wildcards=\"yes\" >CFEngine Community %s Policy Server on %s </name>", Version(), "%h");
#endif
