...
static bool SetOsVersionMinorFromOSRelease(EvalContext *ctx)
{
    ...
    DataType type_out;
    JsonElement *os_release = (JsonElement *) EvalContextVariableGetSpecial(ctx, SPECIAL_SCOPE_SYS, "os_release", &type_out);

    if (os_release == NULL)
    {
        return false;
    }
    JsonElement *version_id = JsonObjectGet(os_release, "VERSION_ID");
    if (version_id == NULL)
    {
        return false;
    }
...
static void SysOsVersionMinor(EvalContext *ctx)
{
    ...
    {
        // 2. if version minor not found, find "OSname_major_minor" hard class with classesmatching
