...
/**
  ...
  @remarks This function invalidates the size of the buffer. Mixing calls to this
           function with other Buffer functions is generally a bad idea. If this
           is needed, then call BufferFix(...) before continuing.
  */
char *BufferGet(Buffer *buffer);

/**
  @brief This function walks the array and fixes the size of the array.
  @param buffer Buffer to be fixed.
  @remarks This functions must be called after calling BufferGet(...) if other
           Buffer functions need to be called.
  */
...
/**
  @brief Ensures buffer is big enough to hold a specified capacity.
  @param buffer Structure to operate on.
  @param capacity New capacity of the buffer.
  @note The capacity can only be increased, trying to set a smaller capacity will
  have no effects on the buffer.
  */
