...
const char *CREATE_TABLE = "CREATE TABLE IF NOT EXISTS data " "(key TEXT PRIMARY KEY, value TEXT NOT NULL)";
...
static int SQLite3_PrepareSchema(sqlite3 *dbp)
{
    ...
    {
        CfOut(cf_error, "", "SQLite3_PrepareSchema: Unable to create table in database: %s", errmsg);
        sqlite3_free(errmsg);
        return false;
    }
...
int SQLite3_WriteComplexKeyDB(sqlite3 *dbp, char *key, int keysize, const void *ptr, int size)
{
    ...
    {
        CfOut(cf_error, "", "SQLite3_WriteComplexKeyDB: Unable to prepare statement: %s", sqlite3_errmsg(dbp));
        goto done;
    }

    if (sqlite3_bind_text(stmt, 1, key, keysize, SQLITE_TRANSIENT) != SQLITE_OK)
    {
        CfOut(cf_error, "", "SQLite3_WriteComplexKeyDB: Unable to bind parameter: %s", sqlite3_errmsg(dbp));
        goto done;
    }
...
int SQLite3_DeleteComplexKeyDB(sqlite3 *dbp, char *key, int keysize)
{
    ...
    {
        CfOut(cf_error, "", "SQLite3_DeleteComplexKeyDB: Unable to prepare statement: %s", sqlite3_errmsg(dbp));
        goto done;
    }

    if (sqlite3_bind_text(stmt, 1, key, keysize, SQLITE_TRANSIENT) != SQLITE_OK)
    {
        CfOut(cf_error, "", "SQLite3_DeleteComplexKeyDB: Unable to bind parameter: %s", sqlite3_errmsg(dbp));
        goto done;
    }
...
void SQLite3_OpenDBTransaction(sqlite3 *dbp)
{
    ...
    {
        CfOut(cf_error, "", "SQLite3_OpenDBTransaction: Unable to begin transaction: %s", errmsg);
        sqlite3_free(errmsg);
    }
}
...
void SQLite3_CommitDBTransaction(sqlite3 *dbp)
{
    ...
    {
        CfOut(cf_error, "", "SQLite3_CommitDBTransaction: Unable to commit: %s", errmsg);
        sqlite3_free(errmsg);
    }
}
