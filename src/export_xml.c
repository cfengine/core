...
XmlStartTag(writer, XMLTAG_FUNCTION, 3,
            fun_name_attr, fun_returntype_attr, fun_varargs_attr);
...
for (i = 0; st[i].btype != NULL; i++)
   ...
   if (strcmp(st[i].subtype, "*") != 0)
      ...
      XmlStartTag(writer,XMLTAG_PROMISETYPE, 2,
                  promise_name_attr, promise_agenttype_attr);
      }
   else
      {
