                                        ...
                                        const Rlist *finalargs)
{
    ...
    if (finalargs == NULL)
    {
        Log(LOG_LEVEL_ERR, "Function %s requires a host name, domain name or IP address as first argument",
            fp->name);
        return FnFailure();
    }
    char *hostnameip = RlistScalarValue(finalargs);
    ...
    char *port = RlistScalarValue(finalargs->next);
    long connect_timeout = (finalargs->next->next != NULL) ? IntFromString(RlistScalarValue(finalargs->next->next)) : 100;
    
    if (connect_timeout > UINT_MAX) {
        Log(LOG_LEVEL_ERR, "Connection timeout is too long");
    }
...
static const FnCallArg ISCONNECTABLE_ARGS[] =
{
    ...
    {CF_VALRANGE, CF_DATA_TYPE_INT, "Connection timeout"},
    {NULL, CF_DATA_TYPE_NONE, NULL}
};
