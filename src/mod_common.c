...
struct BodySyntax CF_TRANSACTION_BODY[] =
   ...
   {"ifelapsed",cf_int,CF_VALRANGE,"Number of minutes before next allowed assessment of promise", "control body value"},
   {"expireafter",cf_int,CF_VALRANGE,"Number of minutes before a repair action is interrupted and retried", "control body value"},
   ...
   {"report_level",cf_opts,"inform,verbose,error,log","The reporting level for standard output for this promise", "none"},
...
struct BodySyntax CF_DEFINECLASS_BODY[] =
   ...
   {"timer_policy",cf_opts,"absolute,reset","Whether a persistent class restarts its counter when rediscovered", "reset"},
   {NULL,cf_notype,NULL,NULL}
   };
...
struct BodySyntax CF_CLASSBODY[] =
   ...
   {"select_class",cf_clist,CF_CLASSRANGE,"Select one of the named list of classes to define based on host identity", "random_selection"},
   {NULL,cf_notype,NULL,NULL}
   };
...
struct BodySyntax CFG_CONTROLBODY[] =
   ...
   {"ignore_missing_bundles",cf_opts,CF_BOOL,"If any bundles in the bundlesequence do not exist, ignore and continue","false"},
   ...
   {"lastseenexpireafter",cf_int,CF_VALRANGE,"Number of minutes after which last-seen entries are purged", "One week"},
   ...
   {"host_licenses_paid",cf_int,CF_VALRANGE,"The number of licenses that you promise to have paid for by setting this value (legally binding for commercial license)", "0"},
   ...
   {"syslog_host",cf_str,CF_IPRANGE,"The name or address of a host to which syslog messages should be sent directly by UDP", "514"},
...
struct BodySyntax CFA_CONTROLBODY[] =
   ...
   {"hashupdates",cf_opts,CF_BOOL,"true/false whether stored hashes are updated when change is detected in source", "false"},
   ...
   {"expireafter",cf_int,CF_VALRANGE,"Global default for time before on-going promise repairs are interrupted", "1 min"},
   ...
   {"ifelapsed",cf_int,CF_VALRANGE,"Global default for time that must elapse before promise will be rechecked", "1"},
   ...
   {"max_children",cf_int,CF_VALRANGE,"Maximum number of background tasks that should be allowed concurrently", "1 concurrent agent promise"},
   {"maxconnections",cf_int,CF_VALRANGE,"Maximum number of outgoing connections to cf-serverd", "30 remote queries"},
   ...
   {"secureinput",cf_opts,CF_BOOL,"true/false check whether input files are writable by unauthorized users", "false"},
   {"sensiblecount",cf_int,CF_VALRANGE,"Minimum number of files a mounted filesystem is expected to have", "2 files"},
   {"sensiblesize",cf_int,CF_VALRANGE,"Minimum number of bytes a mounted filesystem is expected to have", "1000 bytes"},
   {"skipidentify",cf_opts,CF_BOOL,"Do not send IP/name during server connection because address resolution is broken", "false"},
   ...
   {"default_timeout",cf_int,CF_VALRANGE,"Maximum time a network connection should attempt to connect", "10 seconds"},
...
struct BodySyntax CFS_CONTROLBODY[] =
   ...
   {"denybadclocks",cf_opts,CF_BOOL,"true/false accept connections from hosts with clocks that are out of sync", "true"},
   ...
   {"logallconnections",cf_opts,CF_BOOL,"true/false causes the server to log all new connections to syslog", "false"},
   {"logencryptedtransfers",cf_opts,CF_BOOL,"true/false log all successful transfers required to be encrypted", "false"},
   {"maxconnections",cf_int,CF_VALRANGE,"Maximum number of connections that will be accepted by cf-serverd", "30 remote queries"},
...
struct BodySyntax CFM_CONTROLBODY[] =
   {
   {"forgetrate",cf_real,"0,1","Decimal fraction [0,1] weighting of new values over old in 2d-average computation", "0.6"},
...
struct BodySyntax CFRE_CONTROLBODY[] = /* enum cfrecontrol */
   ...
   {"auto_scaling",cf_opts,CF_BOOL,"true/false whether to auto-scale graph output to optimize use of space", "true"},
   {"build_directory",cf_str,".*","The directory in which to generate output files", "Current working directory"},
   ...
   {"html_footer",cf_str,"","HTML code for a page footer to be added to rendered in html before the end body tag"},
   {"query_engine",cf_str,"","Name of a dynamic web-page used to accept and drive queries in a browser"},
   {"reports",cf_olist,"all,audit,performance,all_locks,active_locks,hashes,classes,last_seen,monitor_now,monitor_history,monitor_summary,compliance,setuid,file_changes,installed_software,software_patches,value,variables","A list of reports that may be generated", "none"},
   {"report_output",cf_opts,"csv,html,text,xml","Menu option for generated output format. Applies only to text reports, graph data remain in xydy format.", "none"},
   {"style_sheet",cf_str,"","Name of a style-sheet to be used in rendering html output (added to headers)"},
   {"time_stamps",cf_opts,CF_BOOL,"true/false whether to generate timestamps in the output directory name", "false"},
   {NULL,cf_notype,NULL,NULL}
   };
