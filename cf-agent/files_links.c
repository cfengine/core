...
static int MakeLink(EvalContext *ctx, const char *from, const char *to, Attributes attr, const Promise *pp)
{
    ...
    else
    {
        ...
        {
            cfPS(ctx, OUTPUT_LEVEL_ERROR, PROMISE_RESULT_FAIL, pp, attr, "Couldn't link '%s' to '%s'. (symlink: %s)",
                 to, from, GetErrorStr());
            return false;
        }
        else
...
int MakeHardLink(EvalContext *ctx, const char *from, const char *to, Attributes attr, const Promise *pp)
{
    ...
    else
    {
        if (link(to, from) == -1)
        {
            cfPS(ctx, OUTPUT_LEVEL_ERROR, PROMISE_RESULT_FAIL, pp, attr, "Couldn't (hard) link '%s' to '%s'. (link: %s)",
                 to, from, GetErrorStr());
            return false;
        }
        else
