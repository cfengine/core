...
static JsonElement* VarRefValueToJson(EvalContext *ctx, const FnCall *fp, VarRef *ref, const DataType disallowed_datatypes[], int disallowed_count)
{
    if (NULL == ref)
    {
        return NULL;
    ...
    if (want_type)
    {
        ...
        default:
        {
            ...
            Variable *var;
            while (NULL != (var = VariableTableIteratorNext(iter))
            {
...
static FnCallResult FnCallFold(EvalContext *ctx, ARG_UNUSED const Policy *policy, const FnCall *fp, const Rlist *finalargs)
{
    ...
    JsonElement *json = VarRefValueToJson(ctx, fp, ResolveAndQualifyVarName(fp, name), NULL, 0);
