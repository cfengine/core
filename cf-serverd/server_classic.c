 ...
 */
    ...
    {
        if ((lstat(translated_req_path, &statbuf) == -1) && !(S_ISLNK(statbuf.st_mode))) 
        {
            Log(LOG_LEVEL_INFO, "It is not a dead symbolic link and Couldn't resolve (realpath: %s) filename: %s",
                GetErrorStr(), translated_req_path);
            return false;                /* can't continue without transrequest */
        }
        else
        {
            Log(LOG_LEVEL_VERBOSE, "It is a dead symbolic link, filename: %s", translated_req_path);
            strncpy(transrequest, translated_req_path, CF_BUFSIZE);
        }
    }
