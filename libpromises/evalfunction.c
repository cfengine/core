...
static FnCallResult FnCallHostsWithField(EvalContext *ctx, ARG_UNUSED const Policy *policy, ARG_UNUSED const FnCall *fp, const Rlist *finalargs)
{
    ...
    {
        if (!ListHostsWithField(ctx, &returnlist, field_value, return_format, HOSTSWITHCLASS))
        {
            return FnFailure();
        }
    }
    ...
    {
        if (!ListHostsWithField(ctx, &returnlist, field_value, return_format, HOSTSWITHGROUP))
        {
            return FnFailure();
        }
    }
    ...
    {
        Log(LOG_LEVEL_ERR, "HostswithField with unknown call function %s, aborting", fp->name);
        return FnFailure();
    }
