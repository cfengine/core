 ...
 * network. */
bool PathIsAbsolute(const char *s)
{
    bool result = false;

#if defined(__MINGW32__)
    if (isalpha(s[0]) && (s[1] == ':') && (req_path[2] == FILE_SEPARATOR))
    {
        result = true;
    }
    else
 ...
 *    $(connection.ip), $(connection.key) and $(connection.fqdn)
 ...
 *         return (size_t) -1.
 */
static size_t PreprocessRequestPath(char *reqpath, size_t reqpath_size,
                                    ...
                                    const char *key)
{
    ...
    /* Translate all slashes to backslashes on Winblows so that all the rest
    ...
    if (!PathIsAbsolute(reqpath))
    {
        ...
        char *replacement = StringMapGet(path_shortcuts, reqpath);
