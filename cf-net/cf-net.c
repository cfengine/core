...
static const char *const HINTS[] =
{
    ...
    "Specify maximum CFEngine protocol to use. Possible values: 'classic', 'tls', 'cookie', 'filestream', 'latest' (default)",
    NULL
};
...
// PROTOCOL:
static AgentConnection *CFNetOpenConnection(const char *server, const char *max_protocol_version);
...
static int CFNetConnectSingle(const char *server, const char *max_protocol_version, bool print);
static int CFNetConnect(const char *hostname, const char *max_protocol_version, char **args);
...
static int CFNetMultiTLS(const char *server, const char *max_protocol_version);
...
static void CFNetSetDefault(CFNetOptions *opts){
    ...
    opts->max_protocol_version = NULL;
}
...
static void CFNetOptionsClear(CFNetOptions *opts)
{
    ...
    free(opts->max_protocol_version);
}
    ...
    *hostnames = NULL;
            ...
            != -1)
    {
        switch (c)
        {
            ...
            case 'p':
            {
                opts->max_protocol_version = xstrdup(optarg);
                break;
            }
            default:
            {
                              ...
                              char **args, enum command_enum cmd)
{
    ...
    switch (cmd) {
        case CFNET_CMD_CONNECT:
            return CFNetConnect(hostname, opts->max_protocol_version, args);
        case CFNET_CMD_STAT:
        ...
        case CFNET_CMD_MULTITLS:
            return CFNetMultiTLS(hostname, opts->max_protocol_version);
        default:
            break;
    }
...
static AgentConnection *CFNetOpenConnection(const char *server, const char *max_protocol_version)
{
    ProtocolVersion protocol_version = (max_protocol_version == NULL)
                                     ? CF_PROTOCOL_LATEST
                                     : ParseProtocolVersionPolicy(max_protocol_version);
...
static int CFNetConnectSingle(const char *server, const char *max_protocol_version, bool print)
{
    AgentConnection *conn = CFNetOpenConnection(server, max_protocol_version);
    if (conn == NULL)
    {
        return -1;
    }
...
static int CFNetConnect(const char *hostname, const char *max_protocol_version, char **args)
{
    ...
    CFNetConnectSingle(hostname, max_protocol_version, true);
    return 0;
}
...
static int CFNetStat(ARG_UNUSED CFNetOptions *opts, const char *hostname, char **args)
{
    ...
    char *file = args[1];
    AgentConnection *conn = CFNetOpenConnection(hostname, opts->max_protocol_version);
    if (conn == NULL)
    {
        return -1;
    }
...
typedef struct _GetFileData {
    const char *hostname;
    const char *max_protocol_version;
...
static void *CFNetGetFile(void *arg)
{
    ...
    AgentConnection *conn = CFNetOpenConnection(data->hostname, data->max_protocol_version);
    if (conn == NULL)
    {
        data->ret = false;
        return NULL;
...
static int CFNetGet(ARG_UNUSED CFNetOptions *opts, const char *hostname, char **args)
{
    ...
    for (int i = 0; i < n_threads; i++)
    {
        ...
        threads[i]->data->max_protocol_version = opts->max_protocol_version;
        if (n_threads > 1)
        {
...
static int CFNetMultiTLS(const char *server, const char *max_protocol_version)
{
    ...
    while(ret == 0)
    {
        ret = CFNetConnectSingle(server, max_protocol_version, false);
        ++attempts;
    }
    time_t stop;
    time(&stop);
