...
static bool CanFormatAsStringList(EvalContext *ctx, const Rlist *arg, const Rlist **out)
{
    assert(arg != NULL);

    if (arg->val.type != RVAL_TYPE_SCALAR)
    {
        *out = NULL;
        return false;
    }
        ...
        {
            {
                if (SeqLength(s) >= 2)
                {
                    ...
                    else if (strrchr(format_piece, 'S') != NULL)
                    {
                        ...
                        const Rlist *list;
                        if (CanFormatAsStringList(ctx, arg, &list))
                        {
                            Writer *w = StringWriter();
                            WriterWrite(w, "{ ");
...
static FnCallResult FnCallFileOlderThan(ARG_UNUSED EvalContext *ctx, ARG_UNUSED const Policy *policy, const FnCall *fp, const Rlist *finalargs)
{
    ...
    const char *option = (finalargs->next->next != NULL) ? RlistScalarValue(finalargs->next->next) : "modify";
    ...
    if (StringEqual_IgnoreCase(option, "modify"))
    {
...
static const char *ClassFilterDataGetClassExprFromKey(
    ...
    const char *key_to_class_expr)
{
    JsonElement *json_child = JsonObjectGet(json_object, key_to_class_expr);
    if (json_child == NULL)
    {
        Log(LOG_LEVEL_VERBOSE,
            "Function %s(): Specified key '%s' to class expression was not found in object",
            fn_name, key_to_class_expr);
        return NULL;
    }
    ...
    {
        Log(LOG_LEVEL_VERBOSE,
            "Function %s(): Bad class expression found at given key '%s': "
            "Expected type string, found %s", fn_name, key_to_class_expr,
            JsonGetTypeAsString(json_child));
        return NULL;
    }
    ...
    const char *class_expr_index)
{
    ...
    {
        Log(LOG_LEVEL_VERBOSE,
            "Function %s(): Bad class expression found at given index '%zu': "
            "Expected type string, found type %s",
            fn_name, index, JsonGetTypeAsString(json_array));
        return NULL;
    }
...
static bool ClassFilterDataArrayOfObjects(
    ...
    bool *remove)
{
    const char *class_expr = ClassFilterDataGetClassExprFromKey(fn_name, json_object, key_to_class_expr);
    if (class_expr == NULL)
    {
        /* Error is already logged */
        return false;
    ...
    bool *remove)
{
    if (key_or_index == NULL) {
        Log(LOG_LEVEL_VERBOSE,
            "Function %s(): At least 3 arguments are required when the data structure is an array of objects or an array of arrays. "
            "Please supply a key or index as the third argument.", fn_name);
        return false;
    }
    ...
    case JSON_TYPE_ARRAY:
        ...
        Log(LOG_LEVEL_VERBOSE,
            "Function %s(): Expected child element to be of container type array, found %s",
            fn_name, JsonGetTypeAsString(child));
        break;
    ...
    case JSON_TYPE_OBJECT:
        ...
        Log(LOG_LEVEL_VERBOSE,
            "Function %s(): Expected child element to be of container type object, found %s",
            fn_name, JsonGetTypeAsString(child));
        break;

    default:
        Log(LOG_LEVEL_VERBOSE,
            "Function %s(): Expected child element to be of container type, found %s",
            fn_name, JsonGetTypeAsString(child));
        break;
    }
...
static bool ClassFilterDataObjectOfArrays(
    ...
    const char *fn_name,
    JsonElement *json_array,
    const char *index_to_class_expr,
    const char *key_to_obj,
    bool *remove)
{
    /* Use the key of the object itself as the class expression, unless a class
     * expression index is specified */
    const char *class_expr = key_to_obj;
    if (index_to_class_expr != NULL)
    {
        class_expr = ClassFilterDataGetClassExprFromIndex(fn_name, json_array, index_to_class_expr);
        if (class_expr == NULL)
        {
            /* Error is already logged */
            return false;
...
{
    /* Use the key of the object itself as the class expression, unless a class
     * expression key is specified */
    const char *class_expr = key_to_obj;
    if (key_to_class_expr != NULL)
    {
        class_expr = ClassFilterDataGetClassExprFromKey(fn_name, json_object, key_to_class_expr);
        if (class_expr == NULL)
        {
            /* Error is already logged */
            return false;
...
static bool ClassFilterDataObject(
    EvalContext *ctx,
    const char *fn_name,
    const char *data_structure,
    const char *key_or_index,
    ...
    bool *remove)
{
    ...
    case JSON_TYPE_ARRAY:
        if (StringEqual(data_structure, "auto") ||
            StringEqual(data_structure, "object_of_arrays"))
        {
            return ClassFilterDataObjectOfArrays(
                ctx, fn_name, child, key_or_index, key_to_obj, remove);
        }
        Log(LOG_LEVEL_VERBOSE,
            "Function %s(): Expected child element to be of container type array, found %s",
            fn_name, JsonGetTypeAsString(child));
    ...
    case JSON_TYPE_OBJECT:
        if (StringEqual(data_structure, "auto") ||
            StringEqual(data_structure, "object_of_objects"))
        {
            return ClassFilterDataObjectOfObjects(
                ctx, fn_name, child, key_or_index, key_to_obj, remove);
        }
        Log(LOG_LEVEL_VERBOSE,
            "Function %s(): Expected child element to be of container type object, found %s",
    ...
    const Rlist *args)
{
    ...
    int n_args = 0;
    for (const Rlist *arg = args; arg != NULL; arg = arg->next)
    {
        n_args += 1;
    }
    ...
    {
        Log(LOG_LEVEL_VERBOSE,
            "Function %s(): Require 2 to 3 arguments, got %d",
            fp->name, n_args);
        return FnFailure();
    }

    /* CodeQL does not understand that this is implied from above */
    assert(args != NULL);
    ...
    if (!allocated)
    {
        /* Create heap allocated copy so that we can mutate it */
    ...
    /* This argument can hold:
     * - the index of the class expression in a child array
     * - the key to the class expression in a child object
     * - or NULL (the class expression is the key to the child object)
     */
    const char *key_or_index = (args->next->next != NULL)
                             ? RlistScalarValue(args->next->next)
                             : NULL;

    JsonIterator iter;
    JsonElement *child;
    switch (JsonGetType(parent)) {
        case JSON_TYPE_ARRAY:
            /* Iterate through array in backwards order so that we can avoid
             * having to compute index offsets for each removed element */
            for (size_t index_plus_one = JsonLength(parent); index_plus_one > 0; index_plus_one--)
            {
                ...
                child = JsonArrayGet(parent, index);
                assert(child != NULL);
            ...
            break;
        case JSON_TYPE_OBJECT:
            iter = JsonIteratorInit(parent);
            child = JsonIteratorNextValue(&iter);

            while ((child != NULL))
            {
                const char *key_to_obj = JsonIteratorCurrentKey(&iter);
                assert(key_to_obj != NULL);

                bool remove;
                if (!ClassFilterDataObject(ctx, fp->name, data_structure, key_or_index, child, key_to_obj, &remove))
                {
                    /* Error is already logged */
                    JsonDestroy(parent);
                    return FnFailure();
                ...
                if (remove)
                {
                    NDEBUG_UNUSED bool success = JsonObjectRemoveKey(parent, key_to_obj);
                    assert(success);
                    child = JsonIteratorCurrentValue(&iter);
                }
                else
                {
                    child = JsonIteratorNextValue(&iter);
        ...
        default:
            Log(LOG_LEVEL_VERBOSE,
                "Function %s(): Expected parent element to be of container type array or object, found %s",
...
static const FnCallArg FILE_OLDER_THAN_ARGS[] =
{
    ...
    {"modify,access,change", CF_DATA_TYPE_OPTION, "file timespec"},
    {NULL, CF_DATA_TYPE_NONE, NULL}
};
...
const FnCallType CF_FNCALL_TYPES[] =
{
    FnCallTypeNew("accessedbefore", CF_DATA_TYPE_CONTEXT, ACCESSEDBEFORE_ARGS, &FnCallIsAccessedBefore, "True if arg1 was accessed before arg2 (atime)",
                  FNCALL_OPTION_NONE, FNCALL_CATEGORY_FILES, SYNTAX_STATUS_NORMAL, DEFAULT_ARGC),
    ...
    FnCallTypeNew("and", CF_DATA_TYPE_CONTEXT, AND_ARGS, &FnCallAnd, "Calculate whether all arguments evaluate to true",
                  FNCALL_OPTION_VARARG, FNCALL_CATEGORY_DATA, SYNTAX_STATUS_NORMAL, ARGC(0, -1)),
    ...
    FnCallTypeNew("classfilterdata", CF_DATA_TYPE_CONTAINER, CLASSFILTERDATA_ARGS, &FnCallClassFilterData, "Filter data container by defined classes",
                  FNCALL_OPTION_COLLECTING | FNCALL_OPTION_VARARG, FNCALL_CATEGORY_IO, SYNTAX_STATUS_NORMAL, ARGC(2, 3)),
    FnCallTypeNew("classfiltercsv", CF_DATA_TYPE_CONTAINER, CLASSFILTERCSV_ARGS, &FnCallClassFilterCsv, "Parse a CSV file and create data container filtered by defined classes",
                  FNCALL_OPTION_VARARG, FNCALL_CATEGORY_IO, SYNTAX_STATUS_NORMAL, ARGC(3, 4)),
    ...
    FnCallTypeNew("url_get", CF_DATA_TYPE_CONTAINER, CURL_ARGS, &FnCallUrlGet, "Retrieve the contents at a URL with libcurl",
                  FNCALL_OPTION_NONE, FNCALL_CATEGORY_COMM, SYNTAX_STATUS_NORMAL, DEFAULT_ARGC),
    ...
    FnCallTypeNew("eval", CF_DATA_TYPE_STRING, EVAL_ARGS, &FnCallEval, "Evaluate a mathematical expression",
                  FNCALL_OPTION_VARARG, FNCALL_CATEGORY_DATA, SYNTAX_STATUS_NORMAL, ARGC(1, 3)),
    ...
    FnCallTypeNew("execresult", CF_DATA_TYPE_STRING, EXECRESULT_ARGS, &FnCallExecResult, "Execute named command and assign output to variable",
                  FNCALL_OPTION_CACHED | FNCALL_OPTION_VARARG | FNCALL_OPTION_UNSAFE, FNCALL_CATEGORY_UTILS, SYNTAX_STATUS_NORMAL, ARGC(2, 3)),
    ...
    FnCallTypeNew("filesexist", CF_DATA_TYPE_CONTEXT, FILESEXIST_ARGS, &FnCallFileSexist, "True if the named list of files can ALL be accessed",
                  FNCALL_OPTION_COLLECTING, FNCALL_CATEGORY_FILES, SYNTAX_STATUS_NORMAL, DEFAULT_ARGC),
    ...
    FnCallTypeNew("findfiles", CF_DATA_TYPE_STRING_LIST, FINDFILES_ARGS, &FnCallFindfiles, "Find files matching a shell glob pattern",
                  FNCALL_OPTION_VARARG, FNCALL_CATEGORY_FILES, SYNTAX_STATUS_NORMAL, ARGC(1, -1)),
    ...
    FnCallTypeNew("getenv", CF_DATA_TYPE_STRING, GETENV_ARGS, &FnCallGetEnv, "Return the environment variable named arg1, truncated at arg2 characters",
                  FNCALL_OPTION_NONE, FNCALL_CATEGORY_SYSTEM, SYNTAX_STATUS_NORMAL, DEFAULT_ARGC),
    ...
    FnCallTypeNew("getusers", CF_DATA_TYPE_STRING_LIST, GETUSERS_ARGS, &FnCallGetUsers, "Get a list of all system users defined, minus those names defined in arg1 and uids in arg2",
                  FNCALL_OPTION_VARARG, FNCALL_CATEGORY_SYSTEM, SYNTAX_STATUS_NORMAL, ARGC(0, 2)),
    FnCallTypeNew("getuserinfo", CF_DATA_TYPE_CONTAINER, GETUSERINFO_ARGS, &FnCallGetUserInfo, "Get a data container describing user arg1, defaulting to current user",
                  FNCALL_OPTION_VARARG, FNCALL_CATEGORY_SYSTEM, SYNTAX_STATUS_NORMAL, ARGC(0, 1)),
    ...
    FnCallTypeNew("iprange", CF_DATA_TYPE_CONTEXT, IPRANGE_ARGS, &FnCallIPRange, "True if the current host lies in the range of IP addresses specified in arg1 (can be narrowed to specific interfaces with arg2).",
                  FNCALL_OPTION_VARARG, FNCALL_CATEGORY_COMM, SYNTAX_STATUS_NORMAL, ARGC(1, -1)),
    ...
    FnCallTypeNew("isconnectable", CF_DATA_TYPE_CONTEXT, ISCONNECTABLE_ARGS, &FnCallIsConnectable, "Check if a port is connectable",
                  FNCALL_OPTION_VARARG, FNCALL_CATEGORY_COMM, SYNTAX_STATUS_NORMAL, ARGC(2, 3)),
    ...
    FnCallTypeNew("mapdata", CF_DATA_TYPE_CONTAINER, MAPDATA_ARGS, &FnCallMapData, "Return a data container with each element parsed from a JSON string applied to every key-value pair of the list or array or data container, given as $(this.k) and $(this.v)",
                  FNCALL_OPTION_COLLECTING|FNCALL_OPTION_DELAYED_EVALUATION, FNCALL_CATEGORY_DATA, SYNTAX_STATUS_NORMAL, DEFAULT_ARGC),
    ...
    FnCallTypeNew("mergedata", CF_DATA_TYPE_CONTAINER, MERGEDATA_ARGS, &FnCallMergeData, "Merge two or more items, each a list or array or data container",
                  FNCALL_OPTION_COLLECTING|FNCALL_OPTION_VARARG, FNCALL_CATEGORY_DATA, SYNTAX_STATUS_NORMAL, ARGC(1, -1)),
    ...
    FnCallTypeNew("readfile", CF_DATA_TYPE_STRING, READFILE_ARGS, &FnCallReadFile,       "Read max number of bytes from named file and assign to variable",
                  FNCALL_OPTION_VARARG, FNCALL_CATEGORY_IO, SYNTAX_STATUS_NORMAL, ARGC(1, 2)),
    ...
    FnCallTypeNew("returnszero", CF_DATA_TYPE_CONTEXT, RETURNSZERO_ARGS, &FnCallReturnsZero, "True if named shell command has exit status zero",
                  FNCALL_OPTION_CACHED | FNCALL_OPTION_UNSAFE, FNCALL_CATEGORY_UTILS, SYNTAX_STATUS_NORMAL, DEFAULT_ARGC),
    ...
    FnCallTypeNew("sort", CF_DATA_TYPE_STRING_LIST, SORT_ARGS, &FnCallSort, "Sort a list or array or data container",
                  FNCALL_OPTION_COLLECTING | FNCALL_OPTION_VARARG, FNCALL_CATEGORY_DATA, SYNTAX_STATUS_NORMAL, ARGC(1, 2)),
    ...
    FnCallTypeNew("splitstring", CF_DATA_TYPE_STRING_LIST, SPLITSTRING_ARGS, &FnCallSplitString, "Convert a string in arg1 into a list of max arg3 strings by splitting on a regular expression in arg2",
                  FNCALL_OPTION_NONE, FNCALL_CATEGORY_DATA, SYNTAX_STATUS_DEPRECATED, DEFAULT_ARGC),
    ...
    FnCallTypeNew("usemodule", CF_DATA_TYPE_CONTEXT, USEMODULE_ARGS, &FnCallUseModule, "Execute cfengine module script and set class if successful",
                  FNCALL_OPTION_NONE | FNCALL_OPTION_UNSAFE, FNCALL_CATEGORY_UTILS, SYNTAX_STATUS_NORMAL, DEFAULT_ARGC),
    ...
    FnCallTypeNew("validdata", CF_DATA_TYPE_CONTEXT, VALIDDATA_ARGS, &FnCallValidData, "Check for errors in JSON or YAML data",
                  FNCALL_OPTION_VARARG, FNCALL_CATEGORY_DATA, SYNTAX_STATUS_NORMAL, ARGC(2, 3)),
    ...
    FnCallTypeNew("callstack_promisers", CF_DATA_TYPE_STRING_LIST, CFENGINE_PROMISERS_ARGS, &FnCallCFEngineCallers, "Get the list of promisers to the current promise execution path",
                  FNCALL_OPTION_NONE, FNCALL_CATEGORY_INTERNAL, SYNTAX_STATUS_NORMAL, DEFAULT_ARGC),
