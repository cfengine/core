...
static bool PutVar(VariableTable *table, char *var_str)
{
    ...
    bool ret = VariableTablePut(table, ref, &rval, CF_DATA_TYPE_STRING, NULL);
    VarRefDestroy(ref);
    return ret;
}
...
static VariableTable *ReferenceTable(void)
{
    ...
    assert_false(PutVar(t, "scope2.lval1"));
    {
        ...
        assert_false(VariableTablePut(t, ref, &rval, CF_DATA_TYPE_STRING, NULL));
        VarRefDestroy(ref);
    }
    {
...
static void test_replace(void)
{
    ...
    assert_true(VariableTablePut(t, ref, &rval, CF_DATA_TYPE_STRING, NULL));
