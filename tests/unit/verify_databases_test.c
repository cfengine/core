...
void test_ValidateSQLTableName(void)
{
    ...
    assert_true(ValidateSQLTableName("cfsettings.users", db, table));
    assert_string_equal(db, "cfsettings");
    assert_string_equal(table, "users");

    assert_true(ValidateSQLTableName("a.b", db, table));
    assert_string_equal(db, "a");
    assert_string_equal(table, "b");

    assert_true(ValidateSQLTableName(".b", db, table));
    assert_string_equal(db, "");
    assert_string_equal(table, "b");

    assert_true(ValidateSQLTableName("a.", db, table));
    assert_string_equal(db, "a");
    ...
    strcpy(path, "nosep");
    db[0] = table[0] = '\0';
    assert_false(ValidateSQLTableName(path, db, table));
    assert_string_equal(db, "");
    assert_string_equal(table, "");
