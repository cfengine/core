...
#include <signals.h>            /* HandleSignalsForDaemon() */
...
static void test_event_watcher_lifecycle(void)
{
    ...
    /* No watchers are registered here, and WatcherRegister() doesn't wire up
     * a check_callback for EVENT_FILE_DELETED yet (see the TODO in
     * WatcherRegister()), so there is nothing safe for the watcher thread to
     * poll yet. Force IsPendingTermination() to true up front so
     * WatcherThreadMain() exits its loop immediately instead of polling. */
    HandleSignalsForDaemon(SIGTERM);
    ...
    /* Also joins the watcher thread and finalizes the watcher registry. */
