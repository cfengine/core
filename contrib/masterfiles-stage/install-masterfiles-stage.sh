...
curl --silent ${SRC_DIR}/ssh-wrapper.sh --output ${DC_SCRIPTS_DIR}/ssh-wrapper.sh
curl --silent ${SRC_DIR}/git-askpass.sh --output ${DC_SCRIPTS_DIR}/git-askpass.sh
curl --silent ${SRC_DIR}/example_params/PARAMS_example_git_branch.sh --output ${DEFAULT_PARAMS_FILE}
chown root:root ${DC_SCRIPTS_DIR}/masterfiles-stage.sh \
